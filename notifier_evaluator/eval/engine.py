...
def _resolve_exchange(group: Group, defaults: EngineDefaults) -> str:
    """
    Resolve exchange consistently:
    group.exchange wins
    otherwise engine defaults.exchange
    """
    ex = _safe_strip(getattr(group, "exchange", None)) or _safe_strip(
        getattr(defaults, "exchange", None)
    )
    if not ex:
        ex = ""
    print(
        f"[engine][DBG] resolved_exchange "
        f"group.exchange={getattr(group,'exchange',None)!r} "
        f"defaults.exchange={getattr(defaults,'exchange',None)!r} "
        f"-> {ex!r}"
    )
    return ex
...
def _pick_threshold_condition(
    conditions_in_eval_order: List,
    ...
    def run(self, profiles: List[Profile]) -> RunSummary:
        ...
        for profile in profiles or []:
            ...
            for group in profile.groups or []:
                ...
                if not group.active:
                    print(
                        f"[evaluator][DBG] skip inactive group "
                        f"profile={profile.id} gid={group.gid}"
                    )
                    continue
        ...
        for (profile_id, gid, base_symbol), up in unit_plans.items():
            group = up.group
            resolved_exchange = _resolve_exchange(group, self.cfg.defaults)
            ...
            st = self.store.load_status(status_key)

            if not st.active:
                print(
                    f"[evaluator][DBG] skip inactive status "
                    f"profile={profile_id} gid={gid} symbol={base_symbol}"
                )
                continue
            ...
            for cond in group.conditions:
                ...
                if pair is None:
                    raise ValueError(
                        f"missing resolved pair for rid={cond.rid} "
                        f"profile={profile_id} gid={gid}"
                    )
            ...
            chain = eval_chain(
                cond_results,
                logic_to_prev=_logic_to_prev(eval_conditions),
            )
            ...
            if cond_results:
                ...
                k_left = up.row_map.get(
                    (profile_id, gid, first.rid, base_symbol, "left")
                )
            ...
            threshold_cfg, threshold_rid = _pick_threshold_condition(
                eval_conditions
            )
            ...
            thr = apply_threshold(
                final_state=threshold_target_state,
            ...
            print(
                f"[evaluator][DBG] threshold_strategy=last_condition_with_threshold "
                f"rid={threshold_rid} "
                f"target_state={threshold_target_state.value} "
                f"passed={thr.passed}"
            )
