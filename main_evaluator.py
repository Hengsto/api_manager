...
def _build_engine(*, indicator_base_url: str, status_path: Path, history_path: Path, mapping_path: Path | None) -> EvaluatorEngine:
    defaults = EngineDefaults(
        default_exchange=os.getenv("EVALUATOR_DEFAULT_EXCHANGE", str(getattr(cfg, "DEFAULT_EXCHANGE", "")) or "binance"),
        default_interval=os.getenv("EVALUATOR_DEFAULT_INTERVAL", str(getattr(cfg, "DEFAULT_INTERVAL", "")) or "1h"),
    )
...
def main() -> None:
    ...
    indicator_base_url = args.indicator_base_url or os.getenv("INDICATOR_BASE_URL", f"http://{cfg.MAIN_IP}:{cfg.NOTIFIER_PORT}")
